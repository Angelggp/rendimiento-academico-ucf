...
| Rol | Puede |
|---|---|
| **Administrador** | Crear cuentas de usuario (con cualquier rol — si el rol es Profesor, se le crea el perfil de profesor automático) y mantener el catálogo de asignaturas (nombre, semestre, profesor a cargo y **las carreras que la cursan**). |
...
| **Profesor** | Ver solo sus asignaturas y, dentro de cada una, poner o cambiar la nota (0-5) a los estudiantes **de las carreras que cursan esa asignatura**. La aprobación es automática: 3 o más = Aprobada. |
...
2. **Admin** entra a *Asignaturas* → crea "Redes I", semestre 3, a cargo del profesor recién creado y marcando las carreras que la cursan (ej. Informática).
...
4. Ese **profesor** se loguea, entra a *Mis Asignaturas* → "Redes I", ve los estudiantes de las carreras de esa asignatura (filtrable por carrera) y le pone nota al que acaba de crear la vicedecana (que debe ser de una de esas carreras; de lo contrario el sistema no lo permite). Si pone `2`, la fila queda en estado **Pendiente**; si pone `3` o más, **Aprobada**.
